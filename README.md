...
The illustration uses `d_fun3`. Equality between the scaling parameter
and the item-level scaling function requires assuming the item does not
exhibit DIF on either the slope or intercept, although the function is
more sensitive to intercept DIF than slope DIF.
...
The estimated scaling parameter is approximately 0.38. The first item is
downweighted to zero when estimating the scaling parameter.

## Wald Tests of DIF

Inferences about DIF can be made via Wald tests of whether the
item-level scaling function differs from the estimated scaling
parameter. Using the intended workflow, pass the `rdif` object directly:
...
In this output, `delta` is the estimated scaling parameter subtracted
from the item-level scaling function.
...
functions (i.e., the usual IRT moment-based scaling procedure). The
robust estimator is as above. The null hypothesis that both estimators
are consistent for the “true” scaling parameter leads to the following
test.
...
DIF / Impact.  
In `robustDIF`, the minimization problem is described by `rho_grid`.
...
Note the minimizing value of theta in the plot corresponds to the value
reported by `rdif` above.
...
- First, obtaining item-specific scaling factors $s_i$ analytically from
  $\Sigma_0$ means that we no longer require an ancillary estimate based
  on the scale of the realized values of $Y_i$. This is important
  because it leads the resulting estimator to be highly robust to DIF.
  This is also the main detail that separates the proposed approach from
  that considered (and dismissed) by Stocking and Lord (1984). Stocking
  and Lord used $s = MAD(Y_i)$. However, the MAD has a breakdown point
  of 1/4, so any estimator of $\theta$ that uses $s = MAD(Y_i)$ will
  breakdown if $\geq 1/4$ of the items exhibit DIF (see Huber and
  Roncetti, 2009, chapter 6). By contrast $\Sigma_0$ does not depend
  directly on the scale of the realized values of $Y_i$ – it can be
  computed directly from the item parameters. For some choices of $Y_i$,
  the value of $Y_i$ also appears in the resulting expression for
  $V_0(Y_i)$, which does complicate matters. However, the null
  hypothesis that the item does not exhibit DIF gives
  $Y_i \overset{p}\rightarrow\theta_0$. This motivates using the
  substitution $Y_i = \theta^\star$ when estimating $\Sigma_0$, where
  $\theta^\star$ is a consistent, high-breakdown estimate of $\theta$
  (e.g., the median). The overall result is that the robustness of the
  resulting M-estimator no longer depends on that of an ancillary
  scaling factor.

- Second, using item-specific scaling factors implies that we can
  downweight items with DIF at a desired asymptotic false positive rate
  during IRLS-based estimation. For example, if we choose
  ...
  $\theta$. In this way, DIF detection arises as a by-product of robust
  scaling.
...
Halpin (2024) used the same overall approach to compare two esimates of
