...
# Previous verson for 2pL
# y_intercept2 <- function(mle) {
#    (mle$est$group.2$d1 - mle$est$group.1$d1) / mle$est$group.2$a1
# }
...
# -------------------------------------------------------------------
#' Compute the covariance matrix of the asymptotic null distribution of IRT scaling functions
#'
#'
...
vcov_yz <- function(theta.y, theta.z, mle, log = F) {
  n.items <- nrow(mle$est$group.1)
  n.item.pars <- ncol(mle$est$group.1)
...
#' \dontrun{
#' rdif.intercepts <- rdif(mle = rdif.eg)
#' z_test(theta = rdif.intercepts$est, mle = rdif.eg)
#' }
#'
#' # Test slopes
#' \dontrun{
#' rdif.slopes <- rdif(.mle = rdif.eg, par = "slope")
#' z_test(theta = rdif.slopes$est, mle = rdif.eg, par = "slope")
#' }
#' @export
# -------------------------------------------------------------------
...
# -------------------------------------------------------------------
#' The R-DIF test of all item parameters.
#'
...
rdif_chisq_test<- function(mle, log = F) {
  ...
  vcov.yz <- vcov_yz(theta.y, theta.z, mle, log)
