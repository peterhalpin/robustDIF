...
y_intercept_old <- function(irt.mle) {
...
y_slope_old <- function(irt.mle, log = F) {
...
y_fun_old <- function(irt.mle, par = "intercept", log = F) {
  y <- y_intercept_old(irt.mle)
  if (par == "slope") {
    y <- y_slope_old(irt.mle, log)
  }
  y
}
...
grad_intercept_old <- function(theta, irt.mle) {
  a1 <- irt.mle$par1$a
  delta.a0 <- 0 / a1
  delta.d0 <- - 1 / a1
  delta.a1 <- - theta / a1
...
grad_slope_old <- function(theta, irt.mle, log = F) {
  a <- irt.mle$par0$a
  if (log) {
    theta <- exp(theta)
    a <- irt.mle$par1$a
...
grad_mat_old <- function(grad.vec) {
...
joint_vcov_old <- function(irt.mle) {
...
var_y_old <- function(theta, irt.mle, par = "intercept", log = F) {
  grad.y <- grad_intercept_old(theta, irt.mle)
  if (par == "slope") {
    grad.y <- grad_slope_old(theta, irt.mle, log)
  }
  joint.vcov <- joint_vcov_old(irt.mle)
...
cov_yz_old <- function(theta.y, theta.z, irt.mle, log = F) {
  grad.y <- grad_intercept_old(theta.y, irt.mle)
  grad.z <- grad_slope_old(theta.z, irt.mle, log = log)
  joint.vcov <- joint_vcov_old(irt.mle)
...
get_starts_old <- function(irt.mle, par = "intercept", log = F, alpha = .05){

  y <- y_fun_old(irt.mle, par, log = log)
  var_fun <- function(theta) {var_y_old(theta, irt.mle, par, log = log)}
...
rho_fun_old <- function(irt.mle, par = "intercept", log = F, alpha = .05, grid.width = .05){
...
rdif_old <- function(irt.mle, par = "intercept", log = F, alpha = .05, starting.value = "all", tol = 1e-7, maxit = 100, method = "irls"){
  nit <- 0
  conv <- 1

  # Set up scaling function
  y <- y_fun_old(irt.mle, par, log)
  # Starting value
  starts <- get_starts_old(irt.mle, par, log, alpha)
  theta <- median(starts)
  ...
  # IRLS loop
   while(nit < maxit & conv > tol) {
    var.y <- var_y_old(theta, irt.mle, par, log)
...
z_test_old <- function(theta, irt.mle, par = "intercept", log = F) {
  y <- y_fun_old(irt.mle, par, log)
  var.y <- var_y_old(theta, irt.mle, par, log)
  var.theta <- 1/(sum(1/var.y))
...
chisq_test_old<- function(theta.y, theta.z, irt.mle, log = F) {

  # Set up vectors of Q form
  u <- y_fun_old(irt.mle, par = "intercept") - theta.y
  v <- y_fun_old(irt.mle, par = "slope", log = log) - theta.z
  uv.cbind <- cbind(u, v)
  uv.list <- lapply(1:nrow(uv.cbind), function(i) uv.cbind[i,])
  uv.bdiag <- Matrix::bdiag(uv.list)

  # Set up Sigma matrix of Q form
  var.y <- var_y_old(theta.y, irt.mle, par = "intercept")
  var.theta.y <- 1/sum(1/var.y)
  var.z <- var_y_old(theta.z, irt.mle, par = "slope", log = log)
  var.theta.z <- 1/sum(1/var.z)
  cov.yz <- cov_yz_old(theta.y, theta.z, irt.mle, log = log)
...
-0.02578648 / sqrt(0.02392298) / sqrt(0.07204669)
