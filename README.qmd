...
The estimated scaling parameter is approximately 0.38. The first item is downweighted to zero when estimating the scaling parameter.
...
Using the intended workflow, pass the `rdif` object directly:
...
Note the minimizing value of theta in the plot corresponds to the
value reported by `rdif` above. 
...
-   First, obtaining item-specific scaling factors $s_i$ analytically
    ...
    motivates using the substitution $Y_i = \theta^\star$ when estimating
...
derivation is recapitulated here for general (i.e., non-diagonal)
