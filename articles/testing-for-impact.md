...
Traditionally, researchers compare total test scores or unweighted mean
scores between treatment and control groups. Under certain assumptions,
these test-level treatment effects provide a reasonable proxy of whether
the curriculum impacted the underlying ability or construct the
assessment is intended to measure (referred to as impact; see Halpin, P.
F., & Gilbert, J. B., n.d.). However, this assumes that differences
among assessment items reflect differences in how they measure the same
underlying ability, rather than item-specific effects of the curriculum
itself. Some items may be more sensitive to the curriculum, while others
may show little effect, or even effects in the opposite direction. These
differences are called Item-Level Heterogeneous Treatment Effects
(IL-HTE). When IL-HTE is present, the validity of using aggregate
test-level effects (hereafter, “naïve treatment effects”) as proxies for
impact is threatened and should be further evaluated (Halpin, P. F., &
...
factual recall questions, and even lower performance on questions
...
the treatment effect that is robust to these item-level differences (the
“robust treatment effect”), which reduces the influence of items whose
treatment effects differ significantly from the majority of the
assessment. Items requiring scientific reasoning are highlighted in red,
and we can see that each of them has item-level effects significantly
...
Detecting IL-HTE is important to determine how well treatment effects
...
Before continuing, it is important to explain there are different types
of IL-HTE. Some IL-HTE is construct-relevant, which means the variation
in item-level effects may be scientifically interesting in themselves.
In our hypothetical biology assessment example, suppose questions
regarding scientific reasoning consistently showed larger treatment
effects compared to factual recall or memorization. This might reveal
that the assessment is multidimensional (i.e., is capturing separate
facets of biological expertise), constituting a source of
construct-relevant IL-HTE. Documenting sources of construct-relevant
IL-HTE is useful for refining educational theory (Gilbert et al., 2024b;
Kim et al., 2023), and neglecting to report this type of IL-HTE would
constitute a threat to construct validity because it may indicate that
the curriculum changed how students respond to certain questions.

IL-HTE may be construct-irrelevant if the source of variation in
item-level effects is not due to something meaningful to the treatment.
For example, if the new biology curriculum included lab work with
questions identical to the scientific reasoning items on the assessment,
whereas the old curriculum did not, then students in the treatment group
had prior exposure to exam materials. This would constitute
construct-irrelevant IL-HTE and a threat to internal validity. Whether
IL-HTE is construct-relevant or construct-irrelevant, when only a few
items with strong positive effects drive the test-level treatment
effect, then the treatment may not generalize to assessments without
those items. This is particularly concerning because improvements to
underlying ability (i.e., impact) must generalize to other assessments
