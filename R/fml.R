...
ds.power <- c(power.1, power.2, power.3, power.4, power.5, power.6, power.7, power.8)

ds <- Reduce(rbind, ds.power)
ds <- as.data.frame(ds)
ds <- as.data.frame(lapply(ds, as.numeric))
ds$n = rep(1:8, each = n.reps)
head(ds)
xtable(t(as.matrix(tapply(ds$p, ds$n, function(x) mean(x < .05)))))
ds$reject <- ds$p < .05
# Hist of z-test
ggplot(ds, aes(z.delta, group = reject)) +
  geom_histogram(aes(fill = reject),
                 color = "white",
                 right = TRUE,
                 origin = 0) +
